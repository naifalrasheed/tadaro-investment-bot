...
class TwelveDataAnalyzer:
    ...
    def __init__(self, api_key: str = None):
        ...
        # CIRCUIT BREAKER: Initialize circuit breaker for API failure resilience
        self._init_circuit_breaker()
        logger.info("TwelveData Circuit Breaker initialized for API failure resilience")
    ...
    def _make_request(self, endpoint: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/{endpoint}", params=params, timeout=20)
            self.last_request = time.time()
            ...
            logger.debug(f"TwelveData API success: {endpoint} for {params.get('symbol', 'unknown')}")
